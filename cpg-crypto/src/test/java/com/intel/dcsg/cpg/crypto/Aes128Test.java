...
Output: hello world (str length: 11)
    ...
    public void testEncryptDecrypt() throws CryptographyException, UnsupportedEncodingException {
        ...
        log.info("Input: {} (str length: {})", input, String.valueOf(input.length()) );
        
        SecretKey secretKey = Aes128.generateKey(); // NoSuchAlgorithmException
        log.info("Secret Key: {} (raw length: {})", Base64.encodeBase64String(secretKey.getEncoded()), String.valueOf(secretKey.getEncoded().length) );
        ...
        log.info("Plaintext: {} (raw length: {})", Base64.encodeBase64String(plaintext), String.valueOf(plaintext.length) );
        log.info("Ciphertext: {} (raw length: {})", Base64.encodeBase64String(ciphertext), String.valueOf(ciphertext.length) );
        
        byte[] decrypted = aes.decrypt(ciphertext);
        log.info("Decrypted Plaintext: {} (raw length: {})", Base64.encodeBase64String(decrypted), String.valueOf(decrypted.length) );
        String output = new String(decrypted, "UTF-8");
        log.info("Output: {} (str length: {})", output, String.valueOf(output.length()) );
