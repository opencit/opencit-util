...
public class ConsoleTrustDelegate implements TrustDelegate {
    // TODO:  add the Console object here and the yes/no input prompt methods from setup-console so we can prompt user
    ...
    public ConsoleTrustDelegate(MutablePublicKeyRepository repository) {
        this.repository = new PublicKeyCertificateRepositoryAdapter(repository);
    }

    public static class PublicKeyCertificateRepositoryAdapter implements MutableCertificateRepository {
        private MutablePublicKeyRepository storage;
        public PublicKeyCertificateRepositoryAdapter(MutablePublicKeyRepository storage) {
            this.storage = storage;
        }
        @Override
        public void addCertificate(X509Certificate certificate) throws KeyManagementException {
            storage.addPublicKey(certificate.getPublicKey());
        }
